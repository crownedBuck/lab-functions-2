...
const multiply = (firstNumber, secondNumber, callback) => { // numbers are passing to here
...
first(names, firstName => {
  console.log('The first name in names is ' + firstName) // expect Tyler as name
})
...
const last = (anArray, callback) => {
  callback(anArray[anArray.length - 1]);
}
...
// CODE HERE 
  ...
  const each = (arrayNames, callback) => {
    for (let loopingNumber = 0; loopingNumber < arrayNames.length; loopingNumber++) {
      callback(arrayNames[loopingNumber], loopingNumber)
    }
...
each(names, function namesArray(item, index) {
